...
public class ProfilVersioningService extends AbstractVersioningService<Profil, ProfilRequest> {
    ...
    @Override protected JpaRepository<Profil, Integer> getRepository() { return repository; }
    @Override protected Profil findActiveByTrackingId(UUID id) {
        return repository.findActiveByTrackingId(id).orElse(null); }
    @Override protected UUID getTrackingId(Profil e) { return e.getProfilTrackingId(); }
    @Override protected Profil mapToEntity(ProfilRequest r, Profil existing) {
        Profil p = cloneEntity(existing);
        if (r.profilNom() != null) p.setProfilNom(r.profilNom());
        if (r.commentaire() != null) p.setCommentaire(r.commentaire());
        if (r.organismeTrackingId() != null) {
            Organisme org = organismeRepository.findActiveByTrackingId(r.organismeTrackingId())
                .orElseThrow(() -> new com.ossanasur.cbconnect.exception.RessourceNotFoundException("Organisme introuvable"));
            p.setOrganisme(org);
        }
        if (r.habilitationTrackingIds() != null) {
            List<Habilitation> habs = r.habilitationTrackingIds().stream()
                .map(hid -> habilitationRepository.findActiveByTrackingId(hid)
                    .orElseThrow(() -> new com.ossanasur.cbconnect.exception.RessourceNotFoundException("Habilitation introuvable : " + hid)))

...
public class OrganismeVersioningService extends AbstractVersioningService<Organisme, OrganismeRequest> {
    ...
    protected Organisme mapToEntity(OrganismeRequest r, Organisme existing) {
        ...
        if (r.typeOrganisme() != null)
            u.setTypeOrganisme(r.typeOrganisme());
        if (r.raisonSociale() != null)
            u.setRaisonSociale(r.raisonSociale());
        if (r.code() != null)
            u.setCode(r.code());
        if (r.email() != null)
            u.setEmail(r.email());
        if (r.responsable() != null)
            u.setResponsable(r.responsable());
        if (r.contacts() != null)
            u.setContacts(r.contacts());
        if (r.codePays() != null)
            u.setCodePays(r.codePays());
        if (r.codePaysBCB() != null)
            u.setCodePaysBCB(r.codePaysBCB());
        if (r.paysId() != null)
            u.setPaysId(r.paysId());
        if (r.dateCreation() != null)
            u.setDateCreation(r.dateCreation());
        if (r.numeroAgrement() != null)
            u.setNumeroAgrement(r.numeroAgrement());
        if (r.apiEndpointUrl() != null)
            u.setApiEndpointUrl(r.apiEndpointUrl());

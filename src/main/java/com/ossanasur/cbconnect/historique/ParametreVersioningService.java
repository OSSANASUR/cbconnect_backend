...
public class ParametreVersioningService extends AbstractVersioningService<Parametre, ParametreRequest> {
    ...
    @Override
    protected JpaRepository<Parametre, Integer> getRepository() {
        return repository;
    }

    @Override
    protected Parametre findActiveByTrackingId(UUID id) {
        return repository.findActiveByTrackingId(id).orElse(null);
    }

    @Override
    protected UUID getTrackingId(Parametre e) {
        return e.getParametreTrackingId();
    }

    @Override
    protected Parametre mapToEntity(ParametreRequest r, Parametre existing) {
        Parametre u = cloneEntity(existing);
        if (r.typeParametre() != null)
            u.setTypeParametre(r.typeParametre());
        if (r.cle() != null)
            u.setCle(r.cle());
        if (r.valeur() != null)
            u.setValeur(r.valeur());
    ...
    @Override
    protected void setTrackingId(Parametre entity, UUID newId) {
        entity.setParametreTrackingId(newId);
    }
}

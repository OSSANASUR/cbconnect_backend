...
public class ExpertiseMedicaleServiceImpl implements ExpertiseMedicaleService {
    ...
    public DataResponse<ExpertiseMedicaleResponse> create(ExpertiseMedicaleRequest r, String loginAuteur) {
        ...
        ExpertiseMedicale e = ExpertiseMedicale.builder()
            ...
            .prejudiceEsthetique(r.prejudiceEsthetique()!=null?r.prejudiceEsthetique():QualificationPretium.NEANT)
            ...
            .honorairesContreExpertise(r.honorairesContreExpertise()!=null?r.honorairesContreExpertise():java.math.BigDecimal.ZERO)
            .victime(victime).createdBy(loginAuteur).activeData(true).deletedData(false)
            .fromTable(TypeTable.EXPERTISE_MEDICALE).build();
        if(r.expertTrackingId()!=null) expertRepository.findActiveByTrackingId(r.expertTrackingId()).ifPresent(e::setExpert);
    ...
    public DataResponse<ExpertiseMedicaleResponse> update(UUID id, ExpertiseMedicaleRequest r, String loginAuteur) {
        return DataResponse.success("Expertise mise a jour", mapper.toResponse(versioningService.createVersion(id, r, loginAuteur)));
    }
    ...
    public DataResponse<List<ExpertiseMedicaleResponse>> getByVictime(UUID victimeId) {
        return DataResponse.success(expertiseMedRepository.findByVictime(victimeId).stream().map(mapper::toResponse).collect(Collectors.toList()));
    }
    @Override @Transactional(readOnly=true)
    public DataResponse<List<ExpertiseMedicaleResponse>> getEnAttente() {
        return DataResponse.success(expertiseMedRepository.findSansRapport().stream().map(mapper::toResponse).collect(Collectors.toList()));
    }
    @Override @Transactional

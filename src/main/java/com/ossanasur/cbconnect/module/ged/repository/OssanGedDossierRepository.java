...
public interface OssanGedDossierRepository extends JpaRepository<OssanGedDossier, Integer> {
    ...
    @Query("SELECT d FROM OssanGedDossier d WHERE d.typeDossier='SINISTRE' AND d.activeData=true AND d.deletedData=false ORDER BY d.cheminStockage")
    List<OssanGedDossier> findAllSinistreDossiers();

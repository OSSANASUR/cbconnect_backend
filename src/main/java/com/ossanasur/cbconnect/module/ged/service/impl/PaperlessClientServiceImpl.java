...
public class PaperlessClientServiceImpl implements PaperlessClientService {
    ...
    public void initTagsPaperless() {
        ...
        for (String tag : tags) {
            try { createTagIfAbsent(tag); } catch (Exception e) { log.debug("Tag {} peut-etre existant : {}", tag, e.getMessage()); }
        }
    }
    ...
    public DataResponse<DossierGedResponse> creerDossierSinistre(UUID sinistreId, String loginAuteur) {
        ...
        if (existing.isPresent()) return DataResponse.success("Dossier GED existant", gedMapper.toDossierResponse(existing.get()));
    ...
    public DataResponse<DossierGedResponse> creerDossierVictime(UUID victimeId, String loginAuteur) {
        ...
        if (victime.getSinistre() != null) {
            parentDossier = dossierRepository.findRootBySinistre(victime.getSinistre().getSinistreTrackingId()).orElse(null);
        }

        String nomVictime = victime.getNom().toUpperCase() + "-" + victime.getPrenoms().toUpperCase().replace(" ", "-");
        String chemin = (parentDossier != null ? parentDossier.getCheminStockage() : "BNCB/Sinistres") + "/" + nomVictime;
        ...
        return DataResponse.created("Dossier victime cree", gedMapper.toDossierResponse(dossierRepository.save(dossier)));
    }
    ...
    public DataResponse<DocumentGedResponse> uploadDocument(MultipartFile file, UploadDocumentRequest r, String loginAuteur) {
        ...
        if (dossier == null) throw new RessourceNotFoundException("Dossier GED introuvable, creer le dossier d abord");
        ...
        var sinistre = r.sinistreTrackingId() != null ? sinistreRepository.findActiveByTrackingId(r.sinistreTrackingId()).orElse(null) : null;
        var victime = r.victimeTrackingId() != null ? victimeRepository.findActiveByTrackingId(r.victimeTrackingId()).orElse(null) : null;
    ...
    private String obtainPaperlessToken() throws IOException {
        ...
        String json = "{\"username\":\"" + paperlessConfig.getUsername() + "\",\"password\":\"" + paperlessConfig.getPassword() + "\"}";
    ...
    private Integer uploadToPaperless(MultipartFile file, String title, Integer storagePathId, List<Integer> tagIds) throws IOException {
        ...
        MultipartBody.Builder builder = new MultipartBody.Builder().setType(MultipartBody.FORM)
            .addFormDataPart("document", file.getOriginalFilename(),
                RequestBody.create(file.getBytes(), MediaType.get(file.getContentType() != null ? file.getContentType() : "application/octet-stream")))
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            ...
            if (!response.isSuccessful()) throw new IOException("Paperless upload error " + response.code() + ": " + responseBody);

...
@RestController @RequestMapping("/v1/pays") @RequiredArgsConstructor
@Tag(name="Pays CEDEAO", description="14 Etats membres") @SecurityRequirement(name="bearerAuth")
public class PaysController {
    private final PaysService paysService;
    @GetMapping public ResponseEntity<DataResponse<List<PaysResponse>>> getAll() {
        return ResponseEntity.ok(paysService.getAll()); }
    @GetMapping("/{id}") public ResponseEntity<DataResponse<PaysResponse>> getOne(@PathVariable UUID id) {
        return ResponseEntity.ok(paysService.getByTrackingId(id)); }
    @GetMapping("/iso/{code}") public ResponseEntity<DataResponse<PaysResponse>> getByIso(@PathVariable String code) {
        return ResponseEntity.ok(paysService.getByCodeIso(code)); }
    @PostMapping @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<DataResponse<PaysResponse>> create(@Valid @RequestBody PaysRequest r, @AuthenticationPrincipal UserDetails u) {
        return ResponseEntity.ok(paysService.create(r, u.getUsername())); }
    @PutMapping("/{id}") @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<DataResponse<PaysResponse>> update(@PathVariable UUID id, @Valid @RequestBody PaysRequest r, @AuthenticationPrincipal UserDetails u) {

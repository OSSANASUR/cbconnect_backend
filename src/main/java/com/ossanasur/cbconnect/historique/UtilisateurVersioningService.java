...
public class UtilisateurVersioningService extends AbstractVersioningService<Utilisateur, UtilisateurRequest> {
    ...
    @Override
    protected Utilisateur findActiveByTrackingId(UUID id) {
        return repository.findByUtilisateurTrackingIdAndActiveDataTrueAndDeletedDataFalse(id).orElse(null);
    }

    @Override
    protected UUID getTrackingId(Utilisateur e) {
        return e.getUtilisateurTrackingId();
    }

    @Override
    protected Utilisateur mapToEntity(UtilisateurRequest r, Utilisateur existing) {
    ...
    @Override
    protected void setTrackingId(Utilisateur entity, UUID newId) {
        entity.setUtilisateurTrackingId(newId);
    }
}

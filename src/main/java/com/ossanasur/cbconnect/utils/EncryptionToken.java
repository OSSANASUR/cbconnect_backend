...
public class EncryptionToken {
    @Value("${encryption.secret.key}") private String secretKey;
    ...
    public String encrypt(String s) {
        try { Cipher c = Cipher.getInstance("AES"); c.init(Cipher.ENCRYPT_MODE, getKey());
            return Base64.getEncoder().encodeToString(c.doFinal(s.getBytes(StandardCharsets.UTF_8)));
        } catch (Exception e) { throw new RuntimeException("Erreur de chiffrement", e); }
    }
    public String decrypt(String s) {
        try { Cipher c = Cipher.getInstance("AES"); c.init(Cipher.DECRYPT_MODE, getKey());
            return new String(c.doFinal(Base64.getDecoder().decode(s)), StandardCharsets.UTF_8);
        } catch (Exception e) { throw new RuntimeException("Erreur de dechiffrement", e); }
    }
}

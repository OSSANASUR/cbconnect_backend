...
public class ComptabiliteServiceImpl implements ComptabiliteService {
    ...
    public EcritureResponse genererEcritureAuto(TypeTransactionComptable type, UUID sinistreId, BigDecimal montant, String ref, String loginAuteur) {
    ...
    public DataResponse<EcritureResponse> validerEcriture(UUID id, String loginAuteur) {
        ...
        var validateur = utilisateurRepository.findByEmailAndActiveDataTrueAndDeletedDataFalse(loginAuteur).orElse(null);
    ...
    private BigDecimal resoudreMontant(String expression, BigDecimal montantBase) {
        return switch (expression) {
            ...
            case "FRAIS_GESTION" -> montantBase.multiply(new BigDecimal("0.05")).setScale(2, java.math.RoundingMode.HALF_UP);
            default -> montantBase;
        };
    }
}

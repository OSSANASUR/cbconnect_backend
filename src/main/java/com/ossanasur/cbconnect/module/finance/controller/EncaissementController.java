...
@RestController @RequestMapping("/v1/encaissements") @RequiredArgsConstructor
...
public class EncaissementController {
    ...
    @PostMapping @PreAuthorize("hasAnyRole('SE','COMPTABLE')")
    @Operation(summary="Enregistrer un cheque recu")
    public ResponseEntity<DataResponse<EncaissementResponse>> create(@Valid @RequestBody EncaissementRequest r, @AuthenticationPrincipal UserDetails u) {
    ...
    @PatchMapping("/{id}/encaisser") @PreAuthorize("hasAnyRole('SE','COMPTABLE')")
    ...
    @PatchMapping("/{id}/annuler") @PreAuthorize("hasRole('SE')")
    @Operation(summary="Annuler un encaissement (autorisation SE requise)")
    public ResponseEntity<DataResponse<Void>> annuler(@PathVariable UUID id, @RequestParam String motif, @AuthenticationPrincipal UserDetails u) {

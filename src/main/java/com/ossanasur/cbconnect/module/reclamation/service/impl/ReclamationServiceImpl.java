...
public class ReclamationServiceImpl implements ReclamationService {
    ...
    public DataResponse<DossierReclamationResponse> ouvrirDossier(DossierReclamationRequest r, String loginAuteur) {
        ...
        return DataResponse.created("Dossier reclamation ouvert : " + numero,
                mapper.toResponse(dossierRepository.save(d)));
    }
    ...
    public DataResponse<List<DossierReclamationResponse>> getDossiersByVictime(UUID victimeId) {
        return DataResponse.success(dossierRepository.findByVictime(victimeId).stream().map(mapper::toResponse)
    ...
    @Override
    @Transactional
    public DataResponse<FactureReclamationResponse> validerFacture(UUID id, BigDecimal montantRetenu,
            String loginAuteur) {
        ...
        f.setStatutTraitement(
                montantRetenu.compareTo(f.getMontantReclame()) < 0 ? StatutTraitementFacture.VALIDE_PARTIELLEMENT
                        : StatutTraitementFacture.VALIDE);
        f.setLienAvecAccidentVerifie(true);
    ...
    public DataResponse<List<FactureReclamationResponse>> getFacturesByDossier(UUID dossierId) {
        return DataResponse.success(factureRepository.findByDossier(dossierId).stream().map(mapper::toFactureResponse)
                .collect(Collectors.toList()));
    }

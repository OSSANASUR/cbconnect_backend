...
public class EntiteConstatVersioningService extends AbstractVersioningService<EntiteConstat, EntiteConstatRequest> {
    ...
    @Override
    protected JpaRepository<EntiteConstat, Integer> getRepository() {
        return repository;
    }

    @Override
    protected EntiteConstat findActiveByTrackingId(UUID id) {
        return repository.findActiveByTrackingId(id).orElse(null);
    }

    @Override
    protected UUID getTrackingId(EntiteConstat e) {
        return e.getEntiteConstatTrackingId();
    }

    @Override
    protected EntiteConstat mapToEntity(EntiteConstatRequest r, EntiteConstat ex) {
        ...
        if (r.type() != null)
            u.setType(r.type());
        if (r.localite() != null)
            u.setLocalite(r.localite());
        if (r.codePostal() != null)
            u.setCodePostal(r.codePostal());
        if (r.actif() != null)
            u.setActif(r.actif());
        return u;
    }

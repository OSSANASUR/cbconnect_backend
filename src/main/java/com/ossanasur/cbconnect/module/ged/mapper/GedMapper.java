...
public class GedMapper {
    public DossierGedResponse toDossierResponse(PaperlessDossier d) {
        if(d==null) return null;
        return new DossierGedResponse(
            d.getPaperlessDossierTrackingId(), d.getPaperlessStoragePathId(),
            d.getCheminStockage(), d.getTitre(), d.getTypeDossier(),
            d.getSinistre()!=null?d.getSinistre().getNumeroSinistreLocal():null, null
        );
    }
    public DocumentGedResponse toDocumentResponse(PaperlessDocument doc) {
        if(doc==null) return null;
        return new DocumentGedResponse(
            doc.getPaperlessDocumentTrackingId(), doc.getPaperlessDocumentId(),

...
@Entity @DiscriminatorValue("CHEQUE_RECU_ATTESTATION") @Table(name="cheque_recu_attestation")
public class ChequeRecuAttestation extends InternalHistorique implements Serializable {
    @Column(name="cheque_tracking_id",unique=true) private UUID chequeTrackingId;
    ...
    @Enumerated(EnumType.STRING) @Column(nullable=false) @Builder.Default private StatutCheque statut=StatutCheque.RECU;
    private String motifAnnulation;
    @ManyToOne(fetch=FetchType.LAZY) @JoinColumn(name="facture_id",nullable=false) private FactureAttestation facture;

...
@RestController @RequestMapping("/v1/ged") @RequiredArgsConstructor
@Tag(name="GED Paperless-ngx",description="Proxy Spring Boot vers Paperless-ngx (le frontend ne communique jamais directement avec Paperless)")
@SecurityRequirement(name="bearerAuth")
public class GedController {
    private final PaperlessClientService gedService;
    @PostMapping("/dossiers/sinistre/{sinistreId}")
    @PreAuthorize("hasAnyRole('SE','CSS','REDACTEUR','SECRETAIRE')")
    @Operation(summary="Creer le dossier GED d'un sinistre dans Paperless-ngx")
    ...
    @Operation(summary="Creer le sous-dossier GED d'une victime dans Paperless-ngx")
    ...
    @PreAuthorize("hasAnyRole('SE','CSS','REDACTEUR','SECRETAIRE')")
    @Operation(summary="Uploader un document vers Paperless-ngx via le proxy")
    ...
    @GetMapping("/documents/{paperlessDocumentId}/telecharger")
    @Operation(summary="Telecharger un document depuis Paperless-ngx")
    public ResponseEntity<byte[]> telecharger(@PathVariable Integer paperlessDocumentId) {
        byte[] data = gedService.telechargerDocument(paperlessDocumentId).getData();

...
@RestController
@RequestMapping("/v1/expertises-medicales")
...
public class ExpertiseMedicaleController {
    ...
    public ResponseEntity<DataResponse<ExpertiseMedicaleResponse>> create(
            @Valid @RequestBody ExpertiseMedicaleRequest r, @AuthenticationPrincipal UserDetails u) {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('SE','CSS','REDACTEUR')")
    public ResponseEntity<DataResponse<ExpertiseMedicaleResponse>> update(@PathVariable UUID id,
            @Valid @RequestBody ExpertiseMedicaleRequest r, @AuthenticationPrincipal UserDetails u) {

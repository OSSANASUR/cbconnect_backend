...
public class ExpertServiceImpl implements ExpertService {
    ...
    public DataResponse<ExpertResponse> create(ExpertRequest r, String loginAuteur) {
        Expert e = Expert.builder().expertTrackingId(UUID.randomUUID())
            ...
            .build();
        if(r.paysTrackingId()!=null) paysRepository.findActiveByTrackingId(r.paysTrackingId()).ifPresent(e::setPays);
    ...
    public DataResponse<ExpertResponse> update(UUID id, ExpertRequest r, String loginAuteur) {
        return DataResponse.success("Expert mis a jour", expertMapper.toResponse(versioningService.createVersion(id, r, loginAuteur)));
    }
    ...
    public DataResponse<List<ExpertResponse>> getAllActifsByType(TypeExpert type) {
        return DataResponse.success(expertRepository.findAllActifsByType(type).stream().map(expertMapper::toResponse).collect(Collectors.toList()));
    }
    @Override @Transactional
